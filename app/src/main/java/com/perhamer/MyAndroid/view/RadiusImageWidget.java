...
public class RadiusImageWidget extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public RadiusImageWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RadidsImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.RadidsImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadidsImageView_left_top_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadidsImageView_right_top_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadidsImageView_left_bottom_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadidsImageView_right_bottom_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.RadidsImageView_sriv_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.RadidsImageView_sriv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.RadidsImageView_sriv_oval, false);
        a.recycle();
